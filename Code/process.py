...
import torch
from similarity import FPSearch

def process_file(df: pd.DataFrame, intensity_col: str = "2-P7", smiles_col: str = "SMILES") -> dict:
    """
    Process DataFrame to generate normalized SMILES-intensity dictionary
    ...
    return smiles_intensity_dict

def read_and_process_file(path: str, smiles_col: str = "SMILES") -> Optional[pd.ExcelWriter]:
    """
    Read Excel file, process intensity data, calculate similarity/entropy, and export results
    ...
    for i in target:
        ...
        for name in tqdm(head, desc="Processing intensity columns"):
            smiles_dict = process_file(df, name, smiles_col)
